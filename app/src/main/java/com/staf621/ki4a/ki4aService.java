...
public class ki4aService extends Service {
    ...
    public static List<EndpointItem> endpoints = new ArrayList<>();
    ...
    protected class Wait4connection extends Thread
    {
        protected void reportConnected()
        {
            ...
            showAToast(getString(R.string.text_status_connected)+ " "+Util.ip_domian_string, first_connect);

            MyLog.d(Util.TAG, "Wait4connection: connection detected" + (first_connect ? " [first connection]" : ""));
            if(first_connect) first_connect = false;

            notification.setContentText(getString(R.string.text_status_connected) + Util.ip_domian_string);
            ...
            if (server_text.contains(",")) {
                ...
                for (String host : hosts) {
                    ...
                    try {
                        ...
                        Log.d("Solved host", address.toString());
                        servers.add(host);

                        EndpointItem ep = new EndpointItem();
                        ep.ip = address.toString();
                        ep.host = host;
                        endpoints.add(ep);
                        ...
                        Log.e("SecurityException", e.getMessage(), e);
                    }
                }
            } else {
                servers.add(server_text);
            }
            ...
            while( current_status != Util.STATUS_DISCONNECT && (current_fail_attempt <
                    ...
                    if (preferences.getBoolean("cellular_switch", false)) {
                        ...
                        if (telephonyManager.getDataState() != telephonyManager.DATA_CONNECTED &&
                                ...
                                got_disconnected) {
                            notification.setContentText("Data off");
                            Log.d("data off", server);
                        ...
                        server = server.trim();
                        String ip_show_addr = "";
                        Util.ip_domian_string = "";
                        try {
                            InetAddress our_ip = InetAddress.getByName(server);
                            ip_show_addr = our_ip.toString();
                            Util.ip_domian_string = ip_show_addr;
                    ...
                    Log.d("Trying to connect to: ",  server+ " "+ip_show_addr);
                    //showAToast(String.format(getString(R.string.trying_to_connect),server)+ " "+ip_show_addr,false);
                    ...
                    if (toState == Util.STATUS_SOCKS) {
                        ...
                        ssh_return_val = Util.runChainFireCommand(
                                ((key_switch && !enc_ssh_key) ? "" : BASE + BASE_BIN + "/sshpass -p \"" + password_text + "\" ")
                                        + BASE + BASE_BIN + "/ssh " + server + " -p " + port_number + " -l " + user_text
                    ...
                    notification.setContentText(getString(R.string.text_status_connecting) + " "+ip_show_addr);
                    Resources res = getResources();
                    ...
                    //showAToast(getString(R.string.text_status_reconnecting)+ " "+ip_show_addr , false);
                ...
                // If we have multiple servers let's wait 5 secs before trying to connect to another
                if (current_status != Util.STATUS_DISCONNECT) {
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException e) {
                    ...
                    current_fail_attempt = 0;
                    current_status = Util.STATUS_CONNECTING;
                }
